...
class SevenSegmentDisplay:
    ...
    def __init__(self, address, digit_map, i2cBusId=1, debug=0):
        ...
        if(platform.system() == 'Windows'):
            self.bus = smbus.SMBus(i2cBusId, debug)
            self.bus.resetParticularDevice(address) # to enumlate the real devices which display blank on power on.
        else:
...
segments = {'dp': 0x01, 'a': 0x02, 'b': 0x04, 'c': 0x08, 'd': 0x10, 'e': 0x20, 'f': 0x40, 'g': 0x80}
